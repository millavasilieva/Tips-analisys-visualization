...
tips.insert(0, 'index', value= tips['total_bill'])
...
fig = go.Figure(data=go.Table(
    header=dict(values=list(tips[['index','total_bill', 'tip', 'sex', 'smoker', 'day', 'time','size']].columns), 
        fill_color= 'lemonchiffon',
        align='center'),
    cells=dict(values=[tips.index, tips.total_bill,tips.tip, tips.sex, tips.smoker, tips.day, tips.time,tips['size']],
...
paper_bgcolor=background_color,showlegend=False)
st.write(fig6)
...
# fig7.add_trace(go.Scatter(x='tip', y='total_bill', color='smoker'),
#               row=1, col=1)
# fig7.add_trace(go.Scatter(x='tip', y='total_bill', color='smoker'),
#               row=1, col=1)
